...
impl<T> MonadicReturn<T> for T {
    fn monadic_return(t: T) -> Self {
        t
    }
}
impl<T> MonadicReturn<T> for Option<T> {
    fn monadic_return(t: T) -> Self {
        Some(t)
    }
}
impl<T, E> MonadicReturn<T> for Result<T, E> {
    fn monadic_return(t: T) -> Self {
        Ok(t)
    }
}
impl<T> MonadicReturn<T> for Nondet<T> {
    fn monadic_return(t: T) -> Self {
        Nondet(t)
    }
}
impl<T, E> MonadicReturn<T> for Nondet<Result<T, E>> {
    fn monadic_return(t: T) -> Self {
        Nondet(Ok(t))
    }
}
