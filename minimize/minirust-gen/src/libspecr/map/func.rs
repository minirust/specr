...
impl<K: GcCompat + Clone + Hash + Eq + 'static, V: GcCompat + Clone + 'static> Map<K, V> {
    ...
    pub fn remove(&mut self, k: K) -> Option<V> {
        self.0.mutate(|m| m.remove(&k))
    }

    pub fn contains_key(&self, k: K) -> bool {
        self.0.call_ref_unchecked(|m| m.contains_key(&k))
    }

    pub fn insert(&mut self, k: K, v: V) -> Option<V> {
        self.0.mutate(|m| m.insert(k, v))
    }
